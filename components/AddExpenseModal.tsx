...
const AddTransactionModal: React.FC<AddTransactionModalProps> = ({ isOpen, onClose, transactionToEdit }) => {
    ...
    // Focus on amount field when modal opens for new transactions
    useEffect(() => {
        if (isOpen && !transactionToEdit && amountInputRef.current) {
            ...
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-2">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
                    <div className="relative">
                        ...
                        <input ref={amountInputRef} type="number" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" required className="w-full bg-secondary border border-border rounded-md text-2xl sm:text-3xl font-bold text-right p-3 pr-4 pl-14 sm:pl-16 text-foreground focus:ring-1 focus:ring-ring font-numbers" />
                    </div>
